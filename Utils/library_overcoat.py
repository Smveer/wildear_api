...
def load_pmc(path: string, init: List[int]) -> ctypes.c_void_p:
...
def predict_pmc(mlp_ptr: ctypes.c_void_p, inputs: List[float], is_classification: bool):
    np_input = np.array(inputs, dtype=np.float32)
    ...
    res_ptr = rml.predict_pmc(mlp_ptr, input_ptr, c_length, ctypes.c_bool(c_is_classification))
    return res_ptr[0]
